<template>
  <div class="employee">
     <el-breadcrumb separator="/">
      ...
      <el-breadcrumb-item>建筑垃圾</el-breadcrumb-item>
    </el-breadcrumb>
    <div class="xf-main">
      <div class="xf-left">
      ...
      <div class="xf-right">
       <el-form :inline="true" :model="searchForm" class="demo-form-inline">
        ...
        <div style="margin-top:20px;">
          <el-button type="primary" v-if="arr.some(item =>{return item ==='basic:constructionRefuse:add'})" @click="showDrawer">新增</el-button>
          <el-button type="warning" v-if="arr.some(item =>{return item ==='basic:constructionRefuse:export'})" @click="exportList" style="float: right;">导出</el-button>
        </div>
       ...
       <el-table
            ...
            <el-table-column
              ...
              <template v-slot="scope">
                <div class="operation">
                    <span @click="showEditDialog(scope.row)" v-if="companyIds.some(item =>{ return item ===scope.row.companyId}) && arr.some(item =>{return item ==='basic:constructionRefuse:edit'})">编辑</span>
                    <el-divider direction="vertical" v-if="companyIds.some(item =>{ return item ===scope.row.companyId}) && arr.some(item =>{return item ==='basic:constructionRefuse:edit'})"></el-divider>
                    <span @click="isDelete(scope.row)" v-if="companyIds.some(item =>{ return item ===scope.row.companyId}) && arr.some(item =>{return item ==='basic:constructionRefuse:delete'})">删除</span>
                    <el-divider direction="vertical" v-if="companyIds.some(item =>{ return item ===scope.row.companyId}) && arr.some(item =>{return item ==='basic:constructionRefuse:delete'})"></el-divider>
        ...
        <el-form-item label="建筑垃圾类型：" prop="refuseType">
          <el-select width="200px" v-model="createEmployee.refuseType" placeholder="请选择建筑垃圾类型">
            <el-option
              v-for="item in refuseTypeOption"
              :key="item.value"
              :label="item.name"
              :value="item.value"
            ></el-option>
         ...
         <el-form-item label="建造年份：" prop="buildYear">
          <el-input type="number" v-model.trim="createEmployee.buildYear" placeholder="请输入建造年份"></el-input>
        </el-form-item>
      ...
      <!-- 查看详情抽屉 -->
        <el-drawer
            ...
            <div class="drawer-left">
                  <el-form-item label="所属街道:">{{viewList.streetName}}</el-form-item>
                  <el-form-item label="收集点名称:">{{viewList.name}}</el-form-item>
                  <el-form-item label="建筑垃圾类型:">{{viewList.refuseTypeDesc}}</el-form-item>
                  <el-form-item label="建造年份:">{{viewList.buildYear}}</el-form-item>
...
import { constructionRefuseList, getRegionLocale, constructionRefuseCheck, CompanyList, getEditLog, constructionRefuseExport, getDataFilter, dictItem, constructionRefuseView, constructionDelete, constructionRefuseAdd, constructionRefuseEdit } from '../../api/getData';
    ...
    return {
      ...
      rules: {
        ...
        name: [
          ...
          { validator: this.constructionRefuseCheckName, trigger: ['change', 'blur'] }
        ],
        refuseType: [
          { required: true, message: '请选择建筑垃圾类型', trigger: 'change' }
        ],
        buildYear: [
          { required: true, message: '请填写建造年份', trigger: 'change' },
          { pattern: /^[0-9]*$/, message: '请输入正确的年份', trigger: 'change' }
        ],
  ...
  methods: {
    ...
    export2Excel () {
      import('../../assets/js/Export2Excel').then(excel => {
        ...
        const filterVal = ['companyName', 'streetName', 'name', 'address']
    ...
    constructionRefuseCheckName (rule, value, callback) {
      if (this.title === '新增建筑垃圾') {
        let data = {
          companyId: this.createEmployee.companyId,
          name: value
        }
        constructionRefuseCheck(data).then(res => {
          if (res.data.code === 400) {
            return callback(new Error('该公司下收集点名称已存在'))
          }
          callback()
        })
      } else if (this.title === '编辑建筑垃圾') {
        let data = {
          id: this.rowId,
          companyId: this.createEmployee.companyId,
          name: value
    ...
    showEditDialog (row) {
      ...
      constructionRefuseView(
        ...
        if (res.data.code === 200) {
          ...
          // this.createEmployee.regionLocaleIds = res.data.data.regionLocaleIds
          this.createEmployee.name = res.data.data.name
          this.createEmployee.refuseType = res.data.data.refuseType
          this.createEmployee.buildYear = res.data.data.buildYear
    ...
    showDrawer () {
      ...
      this.createEmployee.refuseType = ''
      this.createEmployee.buildYear = ''
    ...
    isDelete (row) {
      ...
      constructionDelete(data).then(res => {
        console.log(res);
        if (res.data.code === 200) {
        ...
        if (valid) {
          if (this.title === '新增建筑垃圾') {
            let data = {
              ...
              refuseType: this.createEmployee.refuseType,
              buildYear: this.createEmployee.buildYear,
              address: this.createEmployee.address,
              nature: this.createEmployee.nature
            }
            constructionRefuseAdd(data).then(res => {
              if (res.data.code === 200) {
                this.$message({
                  type: 'success',
                  message: '成功!'
            ...
            constructionRefuseEdit(data).then(res => {
              if (res.data.code === 200) {
    ...
    viewDetail (row) {
      ...
      constructionRefuseView(
        ...
        if (res.data.code === 200) {
          this.viewList.streetName = res.data.data.streetName
          this.viewList.name = res.data.data.name
          this.viewList.refuseTypeDesc = res.data.data.refuseTypeDesc
          this.viewList.buildYear = res.data.data.buildYear
    ...
    list () {
      ...
      constructionRefuseList(data).then(res => {
        this.tableData = res.data.data.records
        this.total = res.data.data.total
      })
    },
    dictItemFun () {
      dictItem({ 'code': 'constructionRefuseType', showSelectOption: false }).then(res => {
        this.refuseTypeOption = res.data.data
      })
