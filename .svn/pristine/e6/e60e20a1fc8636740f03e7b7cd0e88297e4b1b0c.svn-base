        ...
        <el-form :inline="true" :model="searchForm" ref="searchForm" class="demo-form-inline">
          <el-row>
            <el-col :span="5">
              ...
              <el-col :span="5">
              <!-- 所属街道 -->
              <el-form-item label="小区名称:" prop="driverLicense">
                  <el-input type="text" placeholder="请输入小区名称" v-model="searchForm.driverLicense"></el-input>
              </el-form-item>
            </el-col>
...
<script>
import { communityPagingList, communityAdd, dataGetRegionLocale, communityView, communityEdit, communityDelete, childCompanyList } from '../../api/getData.js'
export default {
  data () {
    return {
      value: '',
  ...
  created () {
    this.isCommunityPagingList()
    this.isDataGetRegionLocale()
  ...
  methods: {
    ...
    showEditDrawer (id) {
      ...
      return communityView(
        ...
        if (res.data.code === 200) {
          this.editDriver.area = res.data.data.regionLocaleIds
          this.editDriver.cellNumber = res.data.data.code
          this.editDriver.name = res.data.data.name
          this.editDriver.address = res.data.data.address
        }
        console.log(res);
      })
    },
    ...
    isAddCell () {
      return communityAdd(
        {
          'companyId': JSON.parse(localStorage.getItem('login')).companyId,
          'regionLocaleIds': this.createDriver.area[1],
          'code': this.createDriver.cellNumber,
          'address': this.createDriver.address,
          'name': this.createDriver.name
        }
      ).then(res => {
        if (res.data.code === 200) {
          this.isCommunityPagingList()
    ...
    // 获取区域街道列表
    isDataGetRegionLocale () {
      return dataGetRegionLocale(
        {
      ...
      return communityEdit(
        {
          ...
          'id': this.rid,
          'companyId': JSON.parse(localStorage.getItem('login')).companyId,
          'regionLocaleIds': this.editDriver.area[1],
          'code': this.editDriver.cellNumber,
          'address': this.editDriver.address,
    ...
    isSearch () {
      return communityPagingList(
        {
          'companyId': this.searchForm.name,
          'page': this.currentPage,
          'limit': this.pageSize,
          'name': this.searchForm.driverLicense
        }
      ).then(res => {
        console.log(res);
        if (res.data.code === 200) {
