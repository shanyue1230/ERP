  ...
  <div class="LicensInformation">
    ...
    <el-form style="padding-left: 16px;" :inline="true" ref="searchForm" :model="searchForm" class="demo-form-inline" label-position="left" label-width="120px">
          <el-row>
            ...
            <el-col :span="6">
              <!-- 许可名称 -->
              <el-form-item label="许可名称:" prop="licenseName">
                 <el-input v-model="searchForm.licenseName">
                 </el-input>
              </el-form-item>
            </el-col>
              <el-col :span="6">
              <!-- 许可套餐 -->
              <el-form-item label="许可套餐:" prop="packageName">
                 <el-input v-model="searchForm.packageName">
                 </el-input>
              </el-form-item>
            </el-col>
             <el-col :span="6">
              <!-- 有效期至 -->
              <el-form-item label="有效期至:" prop="createTime">
...
<script>
import { licenseMessage } from '../../api/getData.js'
export default {
  data () {
    return {
      searchForm: {
  ...
  created () {
    this.getLicensInformation()
  },
  methods: {
    // 获取许可证信息
    getLicensInformation () {
      licenseMessage().then(res => {
        console.log(res);
        if (res.data.code === 200) {
          // 许可信息回显
          this.searchForm.companyId = res.data.data.companyName
          this.searchForm.createTime = res.data.data.createTime
          this.searchForm.licenseName = res.data.data.licenseName
          this.searchForm.packageName = res.data.data.packageName

          // 套餐信息回显
          console.log(res.data.data);
          this.allData = res.data.data.menus
          console.log(this.allData);

          this.moduleList = res.data.data.menus
    ...
    getMenusList (id) {
      ...
      this.tableData = this.allData[id - 1].menus
      console.log(this.allData[id - 1].menus, id);
